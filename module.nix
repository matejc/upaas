...
let
    ...
    stackServiceFun = name: e:
    {
      ...
      serviceConfig = {
        User = e.user;
        ExecStart = writeScript "${prefix}-stack-${name}-start" "${docker_compose}/bin/docker-compose -p '${e.name}' ${optionalString (e.directory != null) "--project-directory '${e.directory}'"} -f '${e.file}' up --build --no-color";
